...
use crate::{BATCH_SIZE, compression::encodemap::encodemap::EncodeMap, DEFAULT_STEP, MAX_LINE_BLOC, datastruct::tspoint::TsPoint, channels::binarychannel::BinaryChannel, sysutils::schemaurl::SchemaURL};

use super::{indexmap::AppendIndexAnswerKind, IndexMap};
...
impl IndexMap {
    pub fn saves_change(&self) {
        BinaryChannel::write_to_file(&self.path,&BinaryChannel::indexmap_to_bytes(self));
    }
    ...
    pub fn dyn_insert_index_and_data(&mut self, file : &mut File,  point : &TsPoint, insert_pos : &bool) -> bool {
        ...
        let inserted_sample_position = BinaryChannel::append_linecompressed_to_dynfile(file, &EncodeMap::deserialize(&vec![&point.data]));
    ...
    pub fn create(source: &OsString, collection : &OsString) -> Option<IndexMap> {
       if !SchemaURL::get_indexmap_path(source,collection).exists() {
            ...
            BinaryChannel::write_to_file(&path,&BinaryChannel::indexmap_to_bytes(&im) );
            Some(im)
       } else {return None}
    }
